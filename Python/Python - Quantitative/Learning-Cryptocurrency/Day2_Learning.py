...
# =====创建ccxt交易所
p = { 'http': '127.0.0.1:1082', 'https': '127.0.0.1:1082', } # 修改自己的代理端口号即可（具体操作可以查看必看！b圈量化训练营day2新代码操作讲解.PDF）
# exchange = ccxt.binance({'proxies': p}) # 使用币安交议所来抓取数据
exchange = ccxt.okx({'proxies': p})   # 使用okex交议所来抓取数据
# exchange.hostname = ''


# =====设定参数
symbol = 'ETH/USDT'  # 抓取的品种。btc/usdt现货交易对。其他币种照例修改即可，例如ETH/USDT，LTC/USDT等
time_interval = '15m'  # K线的时间周期，其他可以尝试的值：'1m', '5m', '15m', '30m', '1h', '2h', '1d', '1w', '1M', '1y'，并不是每个交易所都支持
...
# =====保存数据到本地
df.to_csv('Yxcccy.csv', index=False)